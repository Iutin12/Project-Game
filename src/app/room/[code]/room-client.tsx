...
export function RoomClient({ code }: { code: string }) {
  ...
  const phaseHint = useMemo(() => {
    ...
    if (room.phase === "DAY_TIE_CHALLENGE") return "После повторной ничьей претенденты проходят короткое испытание.";
    ...
    return (
      <AppShell>
        <section className="mx-auto flex min-h-[70vh] w-full max-w-md flex-col justify-center py-12">
          ...
          <input
            className="mt-8 rounded-xl border border-line bg-white px-4 py-3 text-ink shadow-soft outline-none focus:border-ocean focus:ring-2 focus:ring-ocean/15"
...
function ActionPanel({ room, emitAction }: { room: PublicRoom; emitAction: (event: string, payload?: unknown) => void }) {
  ...
  if (room.phase === "NIGHT_DON" && room.ownRole === "DON") {
    return (
      <div className="space-y-3">
        ...
        >
          {room.donCheckResult ? (
            <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-coral">
  ...
  if (room.phase === "DAY_TIE_CHALLENGE") {
    return <TieChallengePanel room={room} emitAction={emitAction} />;
  }
...
function TieChallengePanel({ room, emitAction }: { room: PublicRoom; emitAction: (event: string, payload?: unknown) => void }) {
  ...
  if (!challenge) {
    return (
      <div className="rounded-[1.5rem] border border-line bg-white/90 p-5 text-slate-600 shadow-soft">
        Готовим испытание для претендентов.
      </div>
    );
  }
      ...
      <div className="mt-4 rounded-2xl border border-line bg-white/70 p-2">
        ...
        {room.phase === "LOBBY" ? (
            ...
            {settingsTab === "vote" ? (
              <div className="grid gap-3">
                ...
                <div className="grid gap-2 sm:grid-cols-3">
                  ...
                  <Button
                    variant={room.settings.voteTieMode === "challenge" ? "primary" : "secondary"}
                    onClick={() => updateSettings({ voteTieMode: "challenge" })}
                  >
                    Испытание
                  </Button>
                  <Button
          ...
          <p className="mt-3 p-2 text-sm leading-6 text-slate-600">
            ...
            {room.settings.voteTieMode === "revote"
              ? "переголосование"
              : room.settings.voteTieMode === "challenge"
                ? "испытание"
                : "никто не выбывает"} · голоса:{" "}
  ...
  return (
    <label className="grid gap-1 text-xs font-medium text-slate-500">
      ...
      <input
        ...
        min={10}
        max={1800}
        className="rounded-xl border border-line bg-white px-2 py-2 text-sm text-ink outline-none focus:border-ocean"
        value={value}
  ...
  return (
    <div className="rounded-[1.5rem] border border-line bg-white/90 p-4 shadow-soft">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="rounded-xl bg-cloud px-2 py-1 text-xs font-semibold text-slate-500">{players.length}</span>
      </div>
      <div className="mt-4 max-h-[22rem] space-y-2 overflow-y-auto overscroll-contain pr-1">
        ...
        {players.map((player) => (
          <div key={player.id} className="flex items-center justify-between gap-3 rounded-2xl border border-line bg-cloud/70 px-3 py-2">
            ...
            <span
              className={[
                "shrink-0 rounded-xl bg-white px-2 py-1 text-xs font-medium",
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-xl bg-cloud px-2 py-1 text-xs font-semibold text-slate-500">{room.chatMessages.length}</span>
      </div>
      <div
        ref={messagesRef}
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {emojis.map((emoji) => (
          <button
            key={emoji}
            type="button"
            className="rounded-xl border border-line bg-cloud px-2 py-1 text-lg hover:bg-slate-100"
      ...
      <div className="mt-3 flex gap-2">
        <input
            className="min-w-0 flex-1 rounded-xl border border-line bg-white px-3 py-3 text-sm text-ink outline-none focus:border-ocean focus:ring-2 focus:ring-ocean/15"
        ...
        />
        <Button type="button" className="px-4" onClick={sendMessage}>
          Отпр.
        </Button>
      </div>
    </div>
  ...
  return (
    <div className="grid h-[24rem] min-h-0 grid-rows-[auto_minmax(0,1fr)_auto] rounded-[1.5rem] border border-slate-300/70 bg-slate-100 p-4 shadow-soft">
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-xl bg-white/80 px-2 py-1 text-xs font-semibold text-slate-500">{room.mafiaChatMessages.length}</span>
      </div>
      ...
      <div className="mt-3 flex gap-2">
        <input
          className="min-w-0 flex-1 rounded-xl border border-slate-300 bg-white px-3 py-3 text-sm text-ink outline-none focus:border-slate-500 focus:ring-2 focus:ring-slate-500/15"

...
function playAliasSignal(kind: "tick" | "last-word") {
  ...
  try {
    ...
    const pulses = kind === "last-word" ? [0, 0.18] : [0];
    pulses.forEach((offset) => {
      const oscillator = context.createOscillator();
      const gain = context.createGain();
      oscillator.type = "sine";
      oscillator.frequency.value = kind === "last-word" ? 880 : 660;
      gain.gain.setValueAtTime(0.0001, startedAt + offset);
      gain.gain.exponentialRampToValueAtTime(0.12, startedAt + offset + 0.01);
...
function TurnActive({ room, emitAction }: { room: PublicAliasRoomState; emitAction: (event: string, payload?: unknown) => void }) {
  ...
  const handleTouchEnd = (endY: number) => {
    ...
    if (difference <= -56) emitAction("alias:word_guessed", { wordId });
    if (difference >= 56 && room.settings.allowSkipWord) emitAction("alias:word_skipped", { wordId });
  };
  return <div className="mx-auto max-w-3xl text-center"><Countdown deadlineAt={turn.deadlineAt} /><Eyebrow>Объясняет {explainer?.name}</Eyebrow>{isExplainer && turn.currentWord ? <><div className="mt-5 touch-pan-x rounded-[1.5rem] border border-coral/30 bg-gradient-to-br from-coral/15 via-white to-orange-50 px-5 py-10 shadow-soft dark:via-slate-950 dark:to-slate-900 sm:py-12" onTouchStart={(event) => { touchStart.current = event.touches[0]?.clientY ?? null; }} onTouchEnd={(event) => handleTouchEnd(event.changedTouches[0]?.clientY ?? 0)}><p className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Секретное слово</p><h2 className="mt-4 max-w-full break-words font-display text-3xl font-semibold leading-tight [overflow-wrap:anywhere] sm:text-6xl">{turn.currentWord.word}</h2><p className="mt-5 text-xs font-bold text-slate-400 sm:hidden">Свайп вверх — угадали, вниз — пропуск</p></div><div className="mt-5 grid grid-cols-2 gap-3"><button className="min-h-24 rounded-[1.25rem] bg-mint px-4 text-xl font-black text-white shadow-soft transition active:scale-[0.98]" onClick={() => emitAction("alias:word_guessed", { wordId })}>УГАДАЛИ<br/><span className="text-sm opacity-80">+1 очко</span></button><button className="min-h-24 rounded-[1.25rem] border border-line bg-white px-4 text-xl font-black text-slate-600 shadow-soft transition active:scale-[0.98] disabled:opacity-40 dark:border-white/10 dark:bg-slate-950 dark:text-white/70" disabled={!room.settings.allowSkipWord} onClick={() => emitAction("alias:word_skipped", { wordId })}>ПРОПУСТИТЬ<br/><span className="text-sm opacity-60">{room.settings.skipPenalty === -1 ? "−1 очко" : "без штрафа"}</span></button></div></> : <Waiting title="Слушайте объясняющего" text="Секретное слово видно только игроку, который сейчас объясняет." />}<div className="mt-5 flex justify-center gap-3"><Badge>Угадано: {turn.guessedCount}</Badge><Badge>Пропущено: {turn.skippedCount}</Badge>{room.players.find((player) => player.id === room.ownPlayerId)?.isHost ? <Button variant="ghost" onClick={() => emitAction("alias:force_finish_turn")}>Завершить ход</Button> : null}</div></div>;
}
...
function LastWord({ room, emitAction }: { room: PublicAliasRoomState; emitAction: (event: string, payload?: unknown) => void }) {
  ...
  useEffect(() => { playAliasSignal("last-word"); }, []);

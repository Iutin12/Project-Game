...
function Scoreboard({ room }: { room: PublicAliasRoomState }) {
  ...
  return <section className={`grid gap-2 ${room.teams.length === 2 ? "grid-cols-2" : "grid-cols-2 lg:grid-cols-4"}`}>{room.teams.map((team) => <article key={team.id} className={`rounded-xl border px-4 py-3 ${currentTeamId === team.id ? `${teamBorder(team.color)} shadow-soft` : "border-line bg-white/70 dark:border-white/10 dark:bg-slate-900/55"}`}><p className="truncate text-xs font-black uppercase tracking-[0.12em] text-slate-500 dark:text-white/50">{team.name}</p><p className="mt-1 text-3xl font-black">{team.score}</p></article>)}</section>;
}
...
function TurnActive({ room, emitAction }: { room: PublicAliasRoomState; emitAction: (event: string, payload?: unknown) => void }) {
  ...
  return <div className="mx-auto max-w-3xl text-center"><Countdown deadlineAt={turn.deadlineAt} /><Eyebrow>Объясняет {explainer?.name}</Eyebrow>{isExplainer && turn.currentWord ? <><div className="mt-5 rounded-[1.5rem] border border-coral/30 bg-gradient-to-br from-coral/15 via-white to-orange-50 px-5 py-12 shadow-soft dark:via-slate-950 dark:to-slate-900"><p className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Секретное слово</p><h2 className="mt-4 font-display text-5xl font-semibold sm:text-7xl">{turn.currentWord.word}</h2></div><div className="mt-5 grid grid-cols-2 gap-3"><button className="min-h-24 rounded-[1.25rem] bg-mint px-4 text-xl font-black text-white shadow-soft transition active:scale-[0.98]" onClick={() => emitAction("alias:word_guessed", { wordId: turn.currentWord!.id })}>УГАДАЛИ<br/><span className="text-sm opacity-80">+1 очко</span></button><button className="min-h-24 rounded-[1.25rem] border border-line bg-white px-4 text-xl font-black text-slate-600 shadow-soft transition active:scale-[0.98] disabled:opacity-40 dark:border-white/10 dark:bg-slate-950 dark:text-white/70" disabled={!room.settings.allowSkipWord || (room.settings.maxSkipsPerTurn !== null && turn.skippedCount >= room.settings.maxSkipsPerTurn)} onClick={() => emitAction("alias:word_skipped", { wordId: turn.currentWord!.id })}>ПРОПУСТИТЬ<br/><span className="text-sm opacity-60">{room.settings.skipPenalty} очков</span></button></div></> : <Waiting title="Слушайте объясняющего" text="Секретное слово видно только игроку, который сейчас объясняет." />}<div className="mt-5 flex justify-center gap-3"><Badge>Угадано: {turn.guessedCount}</Badge><Badge>Пропущено: {turn.skippedCount}</Badge>{room.players.find((player) => player.id === room.ownPlayerId)?.isHost ? <Button variant="ghost" onClick={() => emitAction("alias:force_finish_turn")}>Завершить ход</Button> : null}</div></div>;
}

function LastWord({ room, emitAction }: { room: PublicAliasRoomState; emitAction: (event: string, payload?: unknown) => void }) {
  const turn = room.currentTurn!;
  const own = room.players.find((player) => player.id === room.ownPlayerId);
  const canResolve = turn.explainerPlayerId === room.ownPlayerId || own?.isHost;
  return <div className="mx-auto max-w-3xl text-center"><Eyebrow>Общее слово</Eyebrow><h2 className="mt-2 font-display text-4xl font-semibold">Последний шанс для всех команд</h2>{turn.currentWord ? <div className="mt-5 rounded-[1.5rem] border border-coral/30 bg-coral/10 px-5 py-10"><p className="text-xs font-black uppercase tracking-[0.18em] text-coral">Слово</p><p className="mt-3 font-display text-5xl font-semibold">{turn.currentWord.word}</p></div> : <p className="mt-5 text-slate-500 dark:text-white/55">Слово видит только объясняющий.</p>}{canResolve ? <div className="mt-5 grid gap-2 sm:grid-cols-2">{room.teams.map((team) => <Button key={team.id} variant="secondary" onClick={() => emitAction("alias:last_word_result", { teamId: team.id })}>Угадали: {team.name}</Button>)}<Button variant="ghost" onClick={() => emitAction("alias:last_word_result", {})}>Никто не угадал</Button></div> : <Badge>Ждем результат объясняющего</Badge>}</div>;
}
...
function TurnResult({ room, emitAction }: { room: PublicAliasRoomState; emitAction: (event: string, payload?: unknown) => void }) {
  ...
  const canConfirm = turn.explainerPlayerId === room.ownPlayerId || own?.isHost;
  const showWords = room.settings.showPlayedWords || canReview;
  return <div className="mx-auto max-w-3xl"><Eyebrow>Ход завершен</Eyebrow><div className="flex flex-wrap items-end justify-between gap-3"><h2 className="mt-2 font-display text-4xl font-semibold">Проверьте слова</h2><Badge>Итог: {turn.scoreDelta >= 0 ? "+" : ""}{turn.scoreDelta}</Badge></div><p className="mt-3 text-slate-600 dark:text-white/60">{canReview ? "Нажмите на слово, если оно было отмечено неверно." : "Объясняющий подтверждает итог, после чего ход перейдет следующей команде."}</p>{showWords ? <div className="mt-5 grid gap-2 sm:grid-cols-2">{turn.words.length ? turn.words.map((entry) => <button key={entry.id} disabled={!canReview || entry.id.includes("-last-")} className={`flex items-center justify-between gap-3 rounded-xl border p-4 text-left ${entry.result === "guessed" ? "border-mint/35 bg-mint/10" : "border-coral/30 bg-coral/10"}`} onClick={() => emitAction("alias:toggle_turn_word", { entryId: entry.id })}><span className="font-bold">{entry.word}</span><span className={`font-black ${entry.points > 0 ? "text-mint" : "text-coral"}`}>{entry.points > 0 ? "+" : ""}{entry.points}</span></button>) : <p className="text-slate-400">В этом ходу нет отмеченных слов.</p>}</div> : <p className="mt-5 rounded-xl bg-slate-100 p-4 text-sm text-slate-500 dark:bg-white/5 dark:text-white/50">Список сыгранных слов скрыт настройками комнаты.</p>}{canConfirm ? <Button className="mt-5" onClick={() => emitAction("alias:confirm_turn_result")}>Подтвердить результат</Button> : <div className="mt-5"><Badge>Ждем подтверждения</Badge></div>}</div>;
}
...
function SettingsTab({ room, emitAction }: { room: PublicAliasRoomState; emitAction: (event: string, payload?: unknown) => void }) {
  ...
  return <div className="mt-4 grid gap-4 xl:grid-cols-2"><SettingsSection title="Команды и ход" description="Составы, время объяснения и правила пропусков."><SettingSelect label="Вместимость комнаты" help="Максимальное число игроков, которые смогут войти в лобби." value={String(settings.maxPlayers)} options={[4,6,8,10,12,16,20].map((value) => [String(value),`${value} игроков`])} disabled={disabled} onChange={(value) => update({ maxPlayers: Number(value) })} /><SettingSelect label="Количество команд" help="Игроки распределяются между 2–4 командами." value={String(settings.teamsCount)} options={[["2","2 команды"],["3","3 команды"],["4","4 команды"]]} disabled={disabled} onChange={(value) => update({ teamsCount: Number(value) as 2 | 3 | 4 })} /><Toggle label="Автоматически распределять" help="Сервер равномерно распределит всех подключенных игроков." checked={settings.autoAssignTeams} disabled={disabled} onChange={(value) => update({ autoAssignTeams: value })} /><SettingSelect label="Время хода" help="Время, за которое объясняющий показывает как можно больше слов." value={String(settings.turnTimeSec)} options={[30,45,60,90,120].map((value) => [String(value),`${value} секунд`])} disabled={disabled} onChange={(value) => update({ turnTimeSec: Number(value) as AliasSettings["turnTimeSec"] })} /><Toggle label="Разрешить пропуск" help="Объясняющий может заменить сложное слово следующим." checked={settings.allowSkipWord} disabled={disabled} onChange={(value) => update({ allowSkipWord: value })} /><SettingSelect label="Лимит пропусков" help="Неограниченный режим позволяет пропускать сколько угодно слов за ход." value={settings.maxSkipsPerTurn === null ? "unlimited" : String(settings.maxSkipsPerTurn)} options={[["unlimited","Без ограничений"],["1","1"],["3","3"],["5","5"],["10","10"]]} disabled={disabled || !settings.allowSkipWord} onChange={(value) => update({ maxSkipsPerTurn: value === "unlimited" ? null : Number(value) })} /><SettingSelect label="Штраф за пропуск" help="Счет команды никогда не станет отрицательным." value={String(settings.skipPenalty)} options={[["0","Без штрафа"],["-1","−1 очко"]]} disabled={disabled || !settings.allowSkipWord} onChange={(value) => update({ skipPenalty: Number(value) as 0 | -1 })} /></SettingsSection><SettingsSection title="Победа и результаты" description="Когда партия заканчивается и как фиксируются ответы."><SettingSelect label="Условие победы" help="Игра идет до выбранного счета или заданного числа полных кругов." value={settings.gameEndMode} options={[["score","До счета"],["rounds","По раундам"]]} disabled={disabled} onChange={(value) => update({ gameEndMode: value as AliasSettings["gameEndMode"] })} />{settings.gameEndMode === "score" ? <SettingSelect label="Очков для победы" help="После достижения цели команды доигрывают круг, если включено равенство ходов." value={String(settings.targetScore)} options={[15,20,30,40,50,75,100].map((value) => [String(value),String(value)])} disabled={disabled} onChange={(value) => update({ targetScore: Number(value) })} /> : <SettingSelect label="Количество раундов" help="Один раунд — по одному ходу каждой команды." value={String(settings.roundsCount)} options={[1,2,3,5,7,10].map((value) => [String(value),String(value)])} disabled={disabled} onChange={(value) => update({ roundsCount: Number(value) })} />}<Toggle label="Равное число ходов" help="После достижения целевого счета остальные команды доигрывают текущий круг." checked={settings.equalTurnsAtEnd} disabled={disabled || settings.gameEndMode !== "score"} onChange={(value) => update({ equalTurnsAtEnd: value })} /><SettingSelect label="Последнее слово" help="После таймера последнее слово могут угадывать все команды; объясняющий указывает победителя." value={settings.lastWordMode} options={[["disabled","Выключено"],["common_guess","Общее угадывание"]]} disabled={disabled} onChange={(value) => update({ lastWordMode: value as AliasSettings["lastWordMode"] })} /><Toggle label="Проверять слова после хода" help="Объясняющий может исправить случайно нажатое «угадали» или «пропустить»." checked={settings.reviewWordsAfterTurn} disabled={disabled} onChange={(value) => update({ reviewWordsAfterTurn: value })} /><Toggle label="Показывать сыгранные слова" help="В итогах хода и партии отображается список ответов." checked={settings.showPlayedWords} disabled={disabled} onChange={(value) => update({ showPlayedWords: value })} /></SettingsSection><SettingsSection title="Слова" description="Сложность и тематический набор без повторов внутри партии."><SettingSelect label="Сложность" help="Смешанная сложность использует слова всех уровней." value={settings.difficulty} options={[["mixed","Смешанная"],["easy","Легкая"],["medium","Средняя"],["hard","Сложная"]]} disabled={disabled} onChange={(value) => update({ difficulty: value as AliasSettings["difficulty"] })} /><SettingSelect label="Набор категорий" help="Можно использовать базовый набор или выбрать темы вручную." value={settings.wordPoolMode} options={[["all","Все безопасные категории"],["selected","Выбранные категории"]]} disabled={disabled} onChange={(value) => update({ wordPoolMode: value as AliasSettings["wordPoolMode"] })} />{settings.wordPoolMode === "selected" ? <div className="grid max-h-80 gap-2 overflow-y-auto pr-1 sm:grid-cols-2">{aliasCategories.map((category) => <Toggle key={category} compact label={aliasCategoryLabels[category]} help={category === "adult" ? "Категория 18+ отключена по умолчанию." : undefined} checked={settings.selectedCategories.includes(category)} disabled={disabled} onChange={(checked) => update({ selectedCategories: checked ? [...settings.selectedCategories, category] : settings.selectedCategories.filter((item) => item !== category) })} />)}</div> : null}</SettingsSection><SettingsSection title="Справка" description="Настройки доступны хосту до начала партии."><p className="text-sm leading-6 text-slate-600 dark:text-white/60">Минимум 4 игрока. Секретное слово отправляется только текущему объясняющему. Повторное нажатие по уже обработанному слову сервер отклоняет.</p>{!disabled ? null : <p className="mt-3 rounded-xl bg-slate-100 p-3 text-sm text-slate-500 dark:bg-white/5 dark:text-white/50">{room.phase !== "LOBBY" ? "Настройки заблокированы до следующей партии." : "Настройки может менять только хост."}</p>}</SettingsSection></div>;
}
...
function Countdown({ deadlineAt }: { deadlineAt?: number }) { const [left, setLeft] = useState(() => deadlineAt ? Math.max(0, Math.ceil((deadlineAt - Date.now()) / 1000)) : 0); useEffect(() => { if (!deadlineAt) return; const update = () => setLeft(Math.max(0, Math.ceil((deadlineAt - Date.now()) / 1000))); update(); const timer = window.setInterval(update, 200); return () => window.clearInterval(timer); }, [deadlineAt]); return <div className="mx-auto mb-5 flex w-fit items-center gap-3 rounded-[1.25rem] border border-coral/30 bg-coral/10 px-5 py-3"><span className="h-3 w-3 animate-pulse rounded-full bg-coral"/><strong className="min-w-24 text-3xl text-coral">{Math.floor(left / 60)}:{String(left % 60).padStart(2,"0")}</strong></div>; }
...
function teamBorder(color: AliasTeam["color"]) { return color === "coral" ? "border-coral/40 bg-coral/5" : color === "ocean" ? "border-ocean/40 bg-ocean/5" : color === "mint" ? "border-mint/40 bg-mint/5" : "border-amber-400/40 bg-amber-400/5"; }
